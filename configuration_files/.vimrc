...
" Specify where to backup files (lest there be lots of hidden backup files).
set backupdir=~/Documents/Vim_swap_files_storeroom

" Tell Vim where to put swap files; I set same place, but one need not.
...
" Add one whitespace only, when joining the lines with `J`.
set nojoinspaces
...
" one lower case, be case sensitive, otherwise not.
set smartcase
...
" In NETRW buffer, toggle whether to hide files with `<space>.`.
...
" Use whitespaces instead of tabbed space.
set expandtab

" 1 tabbed space is 3 whitespaces.
set shiftwidth=3
set tabstop=3
...
" Since line-wrapping is set, navigate in terms of display lines
