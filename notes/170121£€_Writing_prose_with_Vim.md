...
$There is a automatically re-formatting command `gqap`, but it only applies to the current paragraph. Maybe the user can define a shortcut for this, but reformatting still need to be done when typing, which is a nuisance.
...
Notice soft wrap is unaffected in both cases (3) and (4). After all, soft wrap avoids single EOL, in which weak and strong rendition differs.

The practice of hard wrap also violates the principle of Markdown somewhat.
The original design of Markdown is easy to write even source code

I mentioned that 
But exactly how long is too long? 
We can probably say that
verage paragraph length 


I usually enlarge the window of iTerm to fit the window, though not full screen.
As I count it, a split window can show 82 halfwidth characters like ASCII, or equivalently, 41 fullwidth characters like Chinese and Chinese punctuations.
The height can fit 33 lines not wrapped. 
This amounts to 2706 halfwidth or 1353 fullwidth characters.

This amounts to 2706 halfwidth or 1353 fullwidth characters.
676.5
A paragraph as long as this is probably too long.

I want to estimate in real time how many characters there are in the current paragraph

Hard wrap requires 

But is still unsatisfying for not smooth scrolling: must begin on the first paragraph

I'd like to 
In iTerm2 adjusted to fit the window but not full window, with ASCII characters shown in 14pt Monaco and Chinese characters in 15pt PingFang SC light, 

## Hard Wrap at Every End of Sentence

$A compromised way is to hard wrap at every end of sentence. This way, every physical line is a sentence, which is a meaningful entity. It thus makes sense to go up, go down, cut, and paste an entire line.

$But nothing is perfect. If hard wrapped at end of every sentence, I cannot estimate the number of words from a block of text, because end of apparent lines are uneven. And such estimation is important, as I decide instantly from the amount of words whether this paragraph is too long, which I should break, or it is too short, which I should combine with adjacent paragraphs.

## Returning to Soft Wrap

$After a long comtemplation on the matter, I decide to return to the original habit, that is, soft wrap.

$Still, I find it useful, when I am in the stage of sketching, to make every sentence a physical line. 
This makes it easy to assembling material by cutting or pasting an entire physical line.
Afterwards, I delete EOLs between physical lines by visual selecting and `J` when writing in English, or `gJ`, in Chinese.
